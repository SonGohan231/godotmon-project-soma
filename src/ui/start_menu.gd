...
func _process(_delta: float) -> void:
	if !visible:
		if Input.is_action_just_pressed("open_menu") && !Observer.dialogue_open:
			open_menu()
		return
...
func _activate_selected() -> void:
	match selected:
		0:
			_show_page("SOMASKANY", "Drużyna\n1. SOMARI   LV 5   HP 48/48\n\nKolejne sloty drużyny zostaną podpięte do trwałego GameState.")
		1:
			_show_page("PLECAK", "PRZEDMIOTY\n• Kapsuła x3\n• Opatrunek x2\n\nKategorie ekwipunku są gotowe do podpięcia do wspólnego stanu gry.")
		2:
			_show_page("TRENER", "TRENER\nPoziom 1\nVela — początek wyprawy\n\nDocelowo tutaj znajdzie się jedna z pięciu dróg rozwoju trenera.")
		3:
			_show_page("ZAPISZ", "System trwałego zapisu powstaje w osobnym module, aby nie uzależniać interfejsu od implementacji danych.")
		4:
			_show_page("OPCJE", "Sterowanie\nPAD — ruch\nA — wybór / interakcja\nZ — cofnięcie\nSTART — menu")
		5:
			close_menu()
