...
func reset_new_game() -> void:
	...
	party = [
		_make_somaskan(&"starter", 5, 48, [&"puls", &"slizg", &"rezonans", &"reset"])
	]
	storage = []
	bag = {
		"capsule": 3,
	...
	quest_flags = {
		"vela_intro": false,
		"starter_received": true,
		"first_wild_battle": false
	}
	world = {
		"map_id": "vela",
		"tile_x": 5,
...
func add_item(item_id: StringName, amount: int = 1) -> int:
	...
	bag[key] = max(0, int(bag.get(key, 0)) + amount)
	return int(bag[key])
