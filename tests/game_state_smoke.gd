...
func _init() -> void:
	...
	if state.party.size() != 1:
		...
		return
	if state.add_item(&"capsule", 2) != 5 || !state.consume_item(&"capsule", 1):
	...
	if int(loaded.world.tile_x) != 11 || int(loaded.world.tile_y) != 7:
		...
		return
	if int(loaded.bag.capsule) != 4 || loaded.party.size() != 2 || String(loaded.trainer.chosen_path) != "badacz":
		_fail("persistent state did not survive save/load")
		return
